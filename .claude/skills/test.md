...
Esempio: `/test agents/categorizer.py`
...
- Per `agents/foo.py` → `agents/tests/test_foo.py`
...
- Un test per comportamento, non per funzione — nomi descrittivi: `test_<cosa>_quando_<condizione>()`
...
**Copertura minima da raggiungere**:
...
**Dopo il 3° tentativo fallito**: fermati, mostra l'output di pytest e spiega cosa blocca il fix — non iterare oltre.
